...
1. **[高] 新增三服务未接线且不持久化**：`DoorService`/`ModelPool`/`RoleCoordinator` 仅被自身测试引用（全仓 grep 证实）；`DoorService` 状态存于内存 Map，重启即失，违背计划对配对持久与心跳状态的要求。
2. **[高] P1-F 工作包遗漏**：准入基准、LanceDB 对比复核、安装器策略、发布包绑定、用户试用均未执行。
3. **[中] 12.4-01 为弱测试**：segments 插入与 coordinator 拒绝之间无因果关系，属拼凑通过；真实注入防线在 s3-orchestration 白名单等处。应删除或改为对真实边界的断言。
4. **[中] P3-A01 与 s2-import 既有覆盖重复**：属回归包装，不应计为新增能力。
5. **[低] P2"隔离安装"以 schema 变更模拟**：不等价于候选包安装演练，未达出口全文要求。

### 69.3 真实环境层事实

门禁默认跳过 12 项真机测试：real-search、memory-eval-real、real-hermes-direct(2)、real-daily-db、real-project-codex(2)、real-hermes-gateway(2)、mcp-bridge、real-https、real-codex。用户接受层：全部未发生。

### 69.4 纠偏后的下一步优先级

1. DoorService 持久化（真实迁移表）+ IPC/UI 接线，或降级声明为协议草案；
2. 执行 P1-F 工作包（含用户试用环节）；
3. ModelPool 接入 ModelProvider 与 extraction 管线并做两真实资源对比；
4. P3 至少一个连接器真实连续增量；
