# IXAEON 隐私与权限模型

> 状态：随里程碑更新。本文件是 v0.1 的隐私承诺基线。

## 核心原则

1. **默认无权限**：没有用户明确选择（原生对话框 / 明确开启），不列目录、不读文件、不采集网页。
2. **可追溯授权**：每个 `source` 都关联一条 `permissions` 记录；撤销授权立即停止后续读取，已导入数据不自动删除（删除由用户明确决定）。
3. **原文不可变**：所有原件按 SHA-256 存入 `vault`，只增不改；纠正和理解变化永不改写历史。
4. **只在本机**：除用户配置的模型 API 外无任何外网请求；无遥测、无统计服务。
5. **最小暴露**：MCP 简报只返回当前项目相关内容；本地 HTTP 只绑定 `127.0.0.1:43191` 且要求 Bearer 令牌。

## 权限范围

| scope_type | 含义 | 典型来源 |
|---|---|---|
| `file` | 单个文件 | 导入对话框选择的文件（mode: once） |
| `folder` | 目录（含子目录，按规则过滤） | 登记的项目根目录（mode: continuous） |
| `domain` | 站点域 | `chatgpt.com`（扩展持续采集，mode: continuous） |

- 路径校验使用 realpath 规范化，防 `..`、符号链接与 junction 逃逸。
- ZIP 导入逐条目校验目标路径，防 Zip Slip。
- 项目目录读取只读说明/配置类文件；排除 `.env*`、`*.pem`、`*.key`、Cookie/Token、`node_modules`、构建产物与二进制文件；单文件 >10MB 需单独确认。

## 项目隔离与全局检索（明确规则）

- **指定项目检索**（问答、`prepare_task`、`search_context` 带 project_ref）：只返回
  `project_id` 严格等于该项目、且 `scope=project` 的资料。`unassigned` / `personal`
  **绝不**自动混入项目上下文。
- **桌面个人视角问答**（不指定项目）：可检索个人与未整理记忆；这是本机用户视角，
  不等于把个人资料交给编码客户端。
- **MCP 编码客户端**：默认只看到 `scope=project` 的项目背景。`personal` /
  `unassigned` 必须有未过期、未撤销的 `disclosure_grants`（受众 `coding_client`）。
  旧 `localToken` 不自动解锁新增个人资料。去掉引用改写成摘要不构成脱敏。
  **原文片段同样适用**：`search_context` 不指定项目时也只检索绑定了项目的来源；
  未绑定项目的来源原文一律不返回，不论是否已被提炼出条目（2026-09-17 修正：
  此前原文检索漏了这道限制，编码客户端不带 project_ref 可搜到个人聊天原文）。
  单独授权的个人条目，其依据原文仍可经 `get_source_excerpt`（按条目引用）展开。
- **授权过滤**：任何检索入口（含全局）都不返回授权已撤销来源的原文。撤权后晚到
  结果不得重新建回已删除或已撤权的有效派生资料。
- **当前对话暂停**（`capture.pausedConversations`）：扩展端本地拦截 + 桌面端服务
  403 双重强制；全局暂停（`capture.enabled=false`）停止一切采集提交。

## 数据离开电脑的渠道（如实，不止一条）

1. **用户配置的 OpenAI 模型 API**（提取 / 问答 / 自动分析，若开启）。发送内容：
   - 提取：当前授权来源的必要片段（按对话轮次/文档标题切块）。
   - 问答：检索到的有限片段（默认 ≤12,000 字符）+ 项目当前结论。
2. **MCP 外部编码客户端**（v0.2 起）：`prepare_task` / `search_context` /
   `get_source_excerpt` 返回的项目简报、条目与原文摘录会交给调用方——通常
   是本机的编码 Agent（如 Claude Code / Codex CLI）。**这些客户端可能把接收
   到的内容再发送给它们自己的云模型**。本地服务只绑定 `127.0.0.1:43191` 且
   要求 Bearer 令牌（localToken），但令牌持有者拿到内容之后的流向不受 IXAEON
   控制。用户应只把 localToken 配置给信任的编码客户端。

两种渠道的边界（如实）：模型 API 调用前界面展示所用来源范围；MCP 调用由
已配置的编码客户端发起，受本地令牌、来源授权和工具参数约束，**IXAEON 不会
在每次 MCP 调用前弹出确认或逐次展示范围**。客户端收到资料后可能转发给其
云模型，请只接入可信客户端。撤销来源授权后简报/摘录/检索立即停止暴露该
来源原文。API Key 使用 Electron `safeStorage` 加密保存（旧版本曾以可解码
格式落盘的密钥在启动时自动迁移或清除，见 0.17/RF08），不进入日志、数据库
明文、错误报告或 Git。

## 未实现事项（如实声明）

- **未实现应用级全库加密**：SQLite 与 vault 文件未整体加密。建议开启 Windows BitLocker 保护磁盘级数据。
- MCP 访问令牌保存在当前 Windows 用户可读的配置中（安装时限制文件 ACL）。

## 扩展权限声明

- 申请 `storage`、`tabs` 两个权限，host 限 `https://chatgpt.com/*` 与 `http://127.0.0.1/*`。
- **`tabs` 的用途**：Service Worker 在休眠唤醒后需要用 `tabs.query` /
  `tabs.sendMessage` 重新找到 chatgpt.com 标签页以恢复采集与暂停状态同步；
  不读取标签页标题、URL 历史或任何非 chatgpt.com 页面内容。
- 不申请 `<all_urls>`、Cookie、history、webRequest；不模拟登录、不调用 ChatGPT 私有接口。
- 只采集当前打开对话中可见的用户消息与 AI 回答；可全局暂停或暂停当前对话。

## 审计

授权授予/撤销、删除派生理解/原始资料、导出、恢复、配对等敏感操作记录 `audit_events`。

## 阻断发布项（v0.1 验收前提）

见《IXAEON_v0.1_开发计划.md》第 8 节。任何一条不满足都不能称为 v0.1 完成。
