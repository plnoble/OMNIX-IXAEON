...
# v0.2.3 发布：理解页连续工作修复（2026-09-08 用户反馈）

## 修复

1. **确认/不采纳后跳回总览**：ItemRow 用 window.location.reload() 整页刷新，
   App 状态重置回默认页。改为 onChanged 回调局部刷新（确认/不采纳/搁置共用），
   处理完一条原地更新继续下一条。
2. **「纠正」点击无反应**：CorrectionDialog 渲染在条目列表之后（页面底部），
   长列表中点击后对话框出现在滚动区外。打开时 scrollIntoView（与设置向导
   同类问题的同款修复）。
3. 搁置按钮原来只改本地 state 不刷新列表，改为同步 onChanged。

## 用户反馈中的正向确认

B 组问答两项通过（真实模型回答正确，暂无错误）。

## 验证

- 新增 UND01（确认后 1.5s 仍在理解页 + 徽标局部更新）/UND02（纠正对话框
  可见 + 完整纠正流程 + 纠正后停留理解页）真实 Electron 界面测试，已纳入
  verify（review-understanding-ui 步骤，共 18 步全绿）。
- desktop e2e 16/16（m2-ui 纠正流程兼容局部刷新）；打包产物复核 3/3。
- GitHub Release v0.2.3 已发布（IXAEON-Setup-0.2.3.exe，122,838,743 字节，
  SHA-256 A77B381E761A4042B2E2EA35262D5A12637EC93FA091E21949BBDBF183CA5F67）；
  0.2.3 win-unpacked 真实网络更新检查返回「已是最新」。
- 如实边界：真实「下载→安装」自动升级路径（如 0.2.2→0.2.3）仍待已装
  0.2.1+ 的机器真机触发一次。
