...
1. `packages/core/src/import/agentSessions.ts` 增加并导出：
   ...
   - 解析规则（锁定的测试就是这些规则的例子）：
     ...
     - `response_item` 且 `payload.type === 'message'`：
       - `role 'user'`：`input_text` 拼起来；以 `# AGENTS.md instructions`、`<environment_context>`、`<user_instructions>`、`<permissions instructions>` 开头的是注入的，跳过；
       - `role 'assistant'`：`output_text` 拼起来，末尾空一行加「〔工具：…〕」，列出从上一条用户消息以来 `function_call` / `custom_tool_call` / `local_shell_call` 的名字（去重、按出现顺序）；
...
2. S1 的 `.jsonl` 导入通道识别到 `'codex'` 时用 `parseCodexSession`。子代理会话（返回 `null` 且 `session_meta` 带 `subagent`）→ `IxaError(VALIDATION_FAILED)`，消息含「子代理」：「这是 Codex 派出去的子代理的会话（如自动审查），不导入」。
