...
function buildPromptBlock(selected: Array<SelectedMemoryItem & { state: string }>): string {
  ...
  const header =
    ...
    `早先记的事可能已经过去或不再成立）：`;
  const adviceNote = includesAiAdvice(selected) ? `\n${AI_ADVICE_NOTE}` : '';
  return `\n\n（${header}${adviceNote}\n${lines.join('\n')}\n）`;
}
 ...
 * 聊天注入与记忆桥（search_memory）用同一句，免得两处说法走样。
 */
export const AI_ADVICE_NOTE =
  '标为「AI 当时的建议」或「用户采纳的 AI 建议」的，是当时的看法，不是定论：' +
  '之后可能有更好的做法，当时也可能考虑不全；有更好的方案就直接提出来，不必照旧执行。';

/** 选出的记忆里有没有 AI 的建议（采纳过的也算）。 */
export function includesAiAdvice(
  items: Array<{ origin: string; saidBy?: 'user' | 'ai' | null }>,
): boolean {
  return items.some((i) => i.saidBy === 'ai' || i.origin === 'assistant_suggestion');
}
