...
export interface IxaIpcApi {
  ...
  // --- P4：Door 设备（桌面端管理面；设备自身走 localServer /api/door/*） ---
  /** 设备视图（不含凭证原文；凭证哈希不下发渲染层）。 */
  listDoorDevices(): Promise<
    Array<{
      id: string;
      name: string;
      platform: string;
  ...
  >;
  /** 配对：rawToken 仅此一次返回，由用户转交设备端保存。 */
  pairDoorDevice(input: {
    name: string;
    platform: 'linux' | 'darwin' | 'win32' | 'android' | 'ios';
    capabilities: string[];
    cpuCores: number;
    totalRamMb: number;
    storageGb: number;
  ...
  revokeDoorDevice(input: { id: string }): Promise<{ ok: true }>;
  evaluateDoorSuitability(input: {
    id: string;
    taskKind: string;
  ...
  dispatchDoorTask(input: {
    deviceId: string;
    taskId: string;
    leaseMs: number;
  }): Promise<{ ok: true; leaseUntil: string }>;
  getDoorTaskLease(
    taskId: string,
  ): Promise<{ taskId: string; deviceId: string; leaseUntil: string; status: string } | null>;
  settleDoorTaskLease(input: { taskId: string; status: string }): Promise<{ ok: true }>;
}
